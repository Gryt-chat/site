...
The bundles need one more change for their voice server, which is further down.
...
The Windows and Linux bundles run the same voice server and don't have that fix yet. Until a bundle does, add these two lines to `config.env` after moving to the 1.10.15 bundle:
